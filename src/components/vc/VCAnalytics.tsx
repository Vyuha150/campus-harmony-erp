...
import {
  Search, Users, GraduationCap, Briefcase, TrendingUp,
  BarChart3, Eye, Award, FlaskConical, BookOpen
} from 'lucide-react';
...
export default function VCAnalytics() {
  ...
  const filtered = departmentPerformance.filter(d => d.department.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Institution-wide performance analytics, department comparisons, and drill-down views</p>
        </div>

        {/* Global Search */}
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input placeholder="Search any student, faculty, department, program..." className="pl-10" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-5">
          {[
            ...
            { label: 'Avg Pass Rate', value: `${(departmentPerformance.reduce((s, d) => s + d.passRate, 0) / departmentPerformance.length).toFixed(1)}%`, icon: BarChart3 },
            { label: 'Avg Placement', value: `${(departmentPerformance.reduce((s, d) => s + d.placementRate, 0) / departmentPerformance.length).toFixed(1)}%`, icon: Briefcase },
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: 'Placement Rate', value: selectedDept?.placementRate },
                  { label: 'Student Satisfaction', value: (selectedDept?.studentSatisfaction || 0) * 20 },
                ].map(metric => (
              ...
              <div className="rounded-lg bg-muted/50 p-3">
                <p className="text-xs text-muted-foreground">
                  Note: For detailed student/faculty records, use the Global Search feature above to find specific individuals.
                </p>
              </div>
            </div>
          </DialogContent>
