...
import { sportsAthletes } from '@/data/sportsMockData';
...
export default function SportsAthletes() {
  ...
  const filtered = sportsAthletes.filter(a => {
    ...
    const matchSport = sportFilter === 'all' || a.sports.some(s => s.sport.toLowerCase() === sportFilter.toLowerCase());
    ...
    <DashboardLayout>
      <div className="space-y-6">
              ...
              <DialogContent className="max-w-lg">
                ...
                <div className="grid gap-4 py-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div><Label>Student ID</Label><Input placeholder="e.g. STU2024001" /></div>
                    <div><Label>Roll Number</Label><Input placeholder="e.g. BT24CS001" /></div>
                  </div>
                  <div><Label>Full Name</Label><Input placeholder="Student name" /></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><Label>Program</Label><Input placeholder="e.g. B.Tech CSE" /></div>
                    <div><Label>Year</Label><Select><SelectTrigger><SelectValue placeholder="Year" /></SelectTrigger><SelectContent><SelectItem value="1">1</SelectItem><SelectItem value="2">2</SelectItem><SelectItem value="3">3</SelectItem><SelectItem value="4">4</SelectItem></SelectContent></Select></div>
                  </div>
                  <div><Label>Primary Sport</Label><Input placeholder="e.g. Cricket, Badminton" /></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><Label>Email</Label><Input type="email" /></div>
                    <div><Label>Phone</Label><Input placeholder="Phone number" /></div>
                  </div>
                  <div><Label>Emergency Contact</Label><Input placeholder="Name – Relationship – Phone" /></div>
                  <Button className="w-full">Register Athlete</Button>
                </div>
              </DialogContent>
            </Dialog>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card><CardContent className="p-4 text-center"><p className="text-2xl font-bold text-foreground">87</p><p className="text-xs text-muted-foreground">Total Athletes</p></CardContent></Card>
          <Card><CardContent className="p-4 text-center"><p className="text-2xl font-bold text-green-600">74</p><p className="text-xs text-muted-foreground">Active</p></CardContent></Card>
          <Card><CardContent className="p-4 text-center"><p className="text-2xl font-bold text-destructive">6</p><p className="text-xs text-muted-foreground">Injured</p></CardContent></Card>
          <Card><CardContent className="p-4 text-center"><p className="text-2xl font-bold text-amber-500">24</p><p className="text-xs text-muted-foreground">Medals This Year</p></CardContent></Card>
        </div>
                ...
                <CardContent className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex gap-2 flex-wrap">{a.sports.map((s, i) => <Badge key={i} variant="outline">{s.sport} ({s.level})</Badge>)}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {a.achievements.length > 0 && <Badge variant="default"><Medal className="mr-1 h-3 w-3" />{a.achievements.length}</Badge>}
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    {athlete.achievements.length === 0 && <p className="text-sm text-muted-foreground">No achievements recorded yet.</p>}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="flex-1"><FileText className="mr-1 h-3 w-3" />Add Achievement</Button>
